...
class LibraryResourceView(LoginRequiredMixin, base.IndexView):
    ...
    def get_context_data(self, resource_id, **kwargs):
        ...
        referenced_resource = models.Resource.objects.get(pk=resource_id)  # url id request
        dimension = resource.dimension
        resources = models.Resource.objects.filter(subcategory__dimension=dimension)
        sections = defaultdict(list)
        for resource in resources:  # This is a list of resource objects filtered on dimension
            sections[resource.subcategory].append(resource)
        context.update({
            'coach': COACHES[dimension.name.upper()],
            'dimension': dimension,
            'sections': dict(sections),
            'resource': resource,  # The incorrect resource from DB
            'referenced_resource': referenced_resource,
            # 'resource_embed': mark_safe(resource.embed) if resource.embed else '',
            'resource_embed': mark_safe(referenced_resource.embed) if referenced_resource.embed else '',
            # 'resource_url': resource.url if not resource.url.endswith('example.com') else ''
            'resource_url': referenced_resource.url if not referenced_resource.url.endswith('example.com') else ''
        })
        return context
